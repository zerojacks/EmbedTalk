...
impl SerialPortChannel {
    pub async fn new(port_name: &str, baud_rate: u32, databit: tokio_serial::DataBits, fowctrl: tokio_serial::FlowControl, parity: tokio_serial::Parity, stopbit: tokio_serial::StopBits) -> Result<Self, Box<dyn Error + Send + Sync>> {
        let serialbuilder = tokio_serial::new(port_name, baud_rate);

        
        let port = SerialStream::open(&serialbuilder
        .data_bits(databit)
...
impl CommunicationChannel for SerialPortChannel {
    ...
    async fn on_disconnect(&self) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        // 发送断开连接事件
        app_handle.emit("channel-disconnected", serde_json::to_string(&payload)?).unwrap();
