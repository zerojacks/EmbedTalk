...
/// 获取通道协议配置
#[command]
pub async fn get_channel_protocol_config(
    channel_id: String,
) -> Result<Option<ChannelProtocolConfig>, String> {
    let channel_handler = get_channel_protocol_handler();
...
/// 使用协议解析数据
#[command]
pub async fn parse_protocol_data(channel_id: String, data: Vec<u8>) -> Result<Value, String> {
    let channel_handler = get_channel_protocol_handler();
    ...
    match channel_handler.parse_data(&channel_id, &data).await {
        ...
        Ok(None) => Err(
            "No protocol configured for this channel or data does not match protocol".to_string(),
        ),
...
pub async fn handle_protocol_message(
    ...
    println!(
        "Protocol: {}, Action: {}, Channel ID: {:?}, Message: {:?}, Params: {:?}",
        protocol, action, channelid, message, params
    );
    // 根据动作类型分发到不同的处理函数
    match action.as_str() {
        "parse" => {
        ...
        "build" => {
            ...
            // 使用协议构建器构建数据
            let bytes = protocol_manager
                .build_with_protocol(&protocol, &params)
                .await
        ...
        "send" => {
            ...
            if config.is_none() {
                // 如果通道未配置协议，则临时配置
                channel_handler
                    .configure_channel(&channel_id, &protocol, params.unwrap_or(json!({})), false)
                    .await
                    .map_err(|e| format!("配置通道协议失败: {}", e))?;
            ...
            channel_handler::send_message(channel_id, bytes, None)
                .await
...
/// 注册协议相关的 Tauri 命令
pub fn register_protocol_commands(
    app: &mut tauri::App,
