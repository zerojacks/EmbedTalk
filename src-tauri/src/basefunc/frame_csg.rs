...
impl FrameCsg {
    ...
    pub fn analysic_csg_frame_by_afn(
        ...
        if frame.len() > 84 {
            if Self::is_contoine_custom_head(&frame[..84]) {
                ...
                Self::analysic_csg_frame_by_afn(&new_frame, result_list, index + 84, region)?;
            }
        }
    ...
    pub fn analysic_csg_head_frame(
        ...
        println!(
            "contro_result: {:?}, result_str: {:?}, dir: {:?}, prm: {:?}",
            contro_result, result_str, dir, prm
        );
        FrameFun::add_data(
            result_list,
        ...
        if all_items.is_empty() {
            ...
            for (i, data_subitem_elem) in all_items.iter().enumerate() {
                ...
                let subitem_name = if let Some(subitem_name_item) = subitem_name_item {
                    subitem_name_item
                        .get_value()
                        .unwrap_or(format!("splitByLength{}", i))
                ...
                if let Some(vaule) = length_map.get(text_part) {
                    ...
                    if let Some(target_results) = target_result {
                        for item in target_results {
                            if let Some(sub_value) = item.get("说明") {
    ...
    pub fn recalculate_sub_length<'a>(
        ...
        if sub_length_cont.to_uppercase() == "UNKNOWN" {
            ...
            {
                // Handle block data
                if data_item_elem.get_attribute("费率数").is_some()
                    || data_item_elem.get_attribute("组数").is_some()
                {
