...
impl FrameCCO {
    ...
    pub fn get_control_code_str(control_data: u8, control_result: &mut Vec<Value>, index: usize) -> (u8, u8, u8, u8) {
        ...
        let dir_str =  if dir == 0 {"下行报文"} else {"上行报文"};
        let prm_str = if prm ==1 {"表示此帧报文来自启动站"} else {"表示此帧报文来自从动站"};
        let add_str = if add == 1 {"表示此帧报文带地址域"} else {"表示此帧报文不带地址域"};
        let ver_str = format!("协议版本号:{}", ver);

        FrameFun::add_data(control_result, "传输方向位DIR".to_string(), dir.to_string(), dir_str.to_string(), vec![index + 0, index + 1], None, None);
        FrameFun::add_data(control_result, "启动标志位PRM".to_string(), prm.to_string(), prm_str.to_string(), vec![index + 0, index + 1], None, None);
        FrameFun::add_data(control_result, "地址域标志位ADD".to_string(), add.to_string(), add_str.to_string(), vec![index + 0, index + 1], None, None);
        FrameFun::add_data(control_result, "协议版本号VER".to_string(), ver.to_string(), ver_str.to_string(), vec![index + 0, index + 1], None, None);
        FrameFun::add_data(control_result, "保留位".to_string(), keep.to_string(), format!("保留位={}",keep), vec![index + 0, index + 1], None, None);

        (dir, prm, add, ver)
    }

    pub fn get_user_data_result(adress_area: &[u8], add: u8, result_list: &mut Vec<Value>, index: usize) {
        ...
        FrameFun::add_data(
            ...
            format!("目的地址:{}", FrameFun::get_data_str_reverser(target_address)),
    ...
    pub fn analysic_cco_frame_by_afn(frame: &[u8], result_list: &mut Vec<Value>, index: usize, region: &str) {
        let (dir, prm, add, afn, pos, mut user_result) = Self::analysic_cco_head_frame(frame, result_list, index);
        
        let app_data = &frame[pos..frame.len()-2];
        let mut app_data_result = Vec::new();
        let protocol = ProtocolInfo::ProtocolCSG16.name().to_string();
        Self::analysic_cco_appdata_frame(app_data, &mut app_data_result, dir, index + pos, &protocol, region);
        ...
        FrameFun::add_data(
            ...
            format!("用户数据:{}", FrameFun::get_data_str_reverser(&frame[4..frame.len()-2])),
    ...
    fn analysic_cco_head_frame(frame: &[u8], result_list: &mut Vec<Value>, index: usize) 
        ...
        let (dir, prm, add, ver) = Self::get_control_code_str(control_data, &mut contro_result, index + 3);
    ...
    fn analysic_cco_di_data(di: &[u8], result: &mut Vec<Value>, index: usize) {
        ...
        let di1_str = format!("功能码类型定义,与AFN值保持一致：{}", Self::get_afn_info(di3, 1, di1));
        let di2_str = format!("报文上下行类型{}", Self::get_direction_str(di2));
        let di3_str = format!("通信双方类型标识:{:02X}-{}", 
            di3, 
            if di3 == 0xE8 { "集中器与本地模块通信" } else { "采集器与本地模块通信" }
        );

        FrameFun::add_data(result, "DI0".to_string(), format!("{:02X}", di0), di0_str, vec![index, index + 1], None, None);
        FrameFun::add_data(result, "DI1".to_string(), format!("{:02X}", di1), di1_str, vec![index + 1, index + 2], None, None);
        FrameFun::add_data(result, "DI2".to_string(), format!("{:02X}", di2), di2_str, vec![index + 2, index + 3], None, None);
        FrameFun::add_data(result, "DI3".to_string(), format!("{:02X}", di3), di3_str, vec![index + 3, index + 4], None, None);
    }

    fn analysic_cco_appdata_frame(data_content: &[u8], result: &mut Vec<Value>, dir: u8, index: usize, protocol: &str, region: &str) {
        ...
        if let Some(data_item_elem) = ProtocolConfigManager::get_config_xml(&data_item, protocol, region, Some(dir)) {
            let pos: usize = 0;
            ...
            let sub_length = if let Some(length_text) = length_ele {
                match length_text.to_uppercase().as_str() {
                    "UNKNOWN" => { FrameCsg::calculate_item_length(&data_item_elem, di_data, protocol, region, Some(dir), None) }
            ...
            FrameFun::add_data(
                ...
                format!("数据内容：{}", FrameFun::get_data_str_reverser(sub_datament)),
    ...
    fn analysic_cco_end_frame(data_content: &[u8], result: &mut Vec<Value>, _dir: u8, index: usize) {
        ...
        FrameFun::add_data(
            ...
            crc_str,
            vec![index + data_content.len() - 2, index + data_content.len() - 1],
            None,
            None
        );
