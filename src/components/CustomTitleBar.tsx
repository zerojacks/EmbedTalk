// src/components/CustomTitleBar.tsx
import React, { useState, useEffect } from 'react';
import { getCurrentWebviewWindow } from '@tauri-apps/api/webviewWindow';
import { FiMinimize, FiMaximize, FiX } from "react-icons/fi";
import reactLogo from "../assets/icons/icon.png";
import { useSettingsContext } from "../context/SettingsProvider"; // 假设上下文在这个路径
import { invoke } from "@tauri-apps/api/core";
import { Theme } from '@tauri-apps/api/window';
import { SettingService } from '../services/settingService';
const appWindow = getCurrentWebviewWindow()

export interface CustomTitleBarProps {
    theme: string;
  }

export const ThemeControl: React.FC = () => {
  const { setTheme, theme } = useSettingsContext();
  const [isChecked, setIsChecked] = useState(theme === 'dark');

  useEffect(() => {
      async function getconfigTheme() {
          try {
            const theme = await SettingService.getTheme();
            if (theme) {
              setcurrentTheme(theme);
            } else {
              setcurrentTheme("system");
            }
          } catch (error) {
            console.error('Failed to get theme setting:', error);
            setcurrentTheme("system");
          }
      };
      getconfigTheme();

  }, []);
  
  async function handleSystemThemeChange(event: MediaQueryListEvent) {
    console.log("systemtheme event", event);
    const isDark = event.matches;
    if (isDark) {
      setTheme("dark");
    } else {
      setTheme("light");
    }
  }

  async function setcurrentTheme(theme: string) {
      let curenttheme = theme;
      if (theme === "system") {
          const isDark = window.matchMedia('(prefers-color-scheme: dark)')
          if (isDark.matches) {
              curenttheme = "dark";
          } else {
              curenttheme = "light";
          }
          const themeMedia = window.matchMedia("(prefers-color-scheme: dark)");
          themeMedia.addEventListener('change', handleSystemThemeChange);
      } 
      setTheme(curenttheme as Theme);
  };

  useEffect(() => {
    setIsChecked(theme === 'dark');
  }, [theme]);

  const handleToggle = () => {
    const newTheme = isChecked ? 'light' : 'dark';
    setTheme(newTheme);
    setIsChecked(!isChecked);
  };

  return (
    <label className="swap swap-rotate">
      {/* Checkbox that controls the theme */}
      <input type="checkbox" checked={isChecked} onChange={handleToggle} />

      {/* Sun icon */}
      <svg className="swap-on fill-current w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        <path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
      </svg>

      {/* Moon icon */}
      <svg className="swap-off fill-current w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        <path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
      </svg>
    </label>
  );
};
export const CustomTitleBar: React.FC<CustomTitleBarProps> = ({ theme }) => {  
  const handleClose = () => {
    localStorage.clear()
    invoke('app_close');
  };

    return (
      <>
        <div className="flex justify-between items-center w-full h-10 bg-base-200" data-tauri-drag-region>
          <div className="flex items-center space-x-2">
            <img src={reactLogo} alt="Logo" className="h-6 m-2" />
            <span className="text-lg font-semibold">EmbedTalk</span>
          </div>
          <div className="flex space-x-2">
            <ThemeControl />
            <button onClick={() => appWindow.minimize()} className="hover:bg-gray-700 p-2 rounded">
              <FiMinimize />
            </button>
            <button onClick={() => appWindow.toggleMaximize()} className="hover:bg-gray-700 p-2 rounded" data-tauri-maximize-button-region>
              <FiMaximize />
            </button>
            <button onClick={() => handleClose()} className="hover:bg-red-600 p-2 rounded">
              <FiX />
            </button>
          </div>
        </div>
      </>
    );
  };